...
This directory contains the code used to prepare labelled training dataset, train a model to distinguish formal and informal neighborhoods in urban context, and perfom inference in an area of interest (AOI).

## Training Data Preparation

This model is created and trained to distinguish formal and informal *50x50m large urban tiles*. The tiles are described by the following four attributes:
...
To train the model a substantial amount of tiles must be identified as formal and informal for the training and validation purposes. To obtain informal tiles, the publicly available [Shelter Associates](https://app.shelter-associates.org/) was used, which as an organization surveys infromal settlements inside the State of Maharashtra and provides informal settlement boundaries. These boundaries are used to create the informal settlement part of the labelled data set. Some informal settlement boundaries expand to forests. Due to use of satellite data, which can not penetrate dense canopy these parts of the informal settlements appear empty and as such as unusable for the model training, therefore such areas are ignored. The rest of the shapes are sliced into the 50x50m tiles based on their inner grid. For Nairobi, informal settlement boundaries were obtained from [Slum Dwellers International](https://sdinet.org/explore-our-data/country/?country=Kenya) and [Spatial Collective](https://github.com/SCollective/Datasets/tree/master).


To create a similar amount of urban formal settlement tiles several urban parts of Navi Mumbai, Pune, Thane, and Nairobi are selected, which were not mapped by any of three sources as informal. These parts of the cities are divided into 50x50m tiles and serve as the formal settlement part of the labelled data set.

## Model Training

A Feed-Forward Neural Network architecture with two hidden layers and an output layer was created:
...
The follwing activation functions were used: 
- Hidden layers: ReLU
- Output layer: Sigmoid

Keras Tuner was used to automatically search for the best hyperparameters, combined with early stopping to reduce overfitting. The model was trained using 50 epochs, using Adam's optimizer.
...
Once trained, the best model was saved and used to predict informal settlements in urban areas in india and Kenya. The same data preparation and preprocessing steps used during training are applied before using the trained model to generate predictions. [GHS-SMOD](https://human-settlement.emergency.copernicus.eu/download.php?ds=smod) data from the Global Human Settlement Layer is used to identify urban areas. 
...
1.	1.0 preparation_of_dataset.ipynb
2.	2.0. training_to_parquet.ipynb
3.	3.0. NN_withoutTrees.ipynb
